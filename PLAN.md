...
Execute the remaining backlog in this order: (1) validate long-run continuity on phone/work-laptop hardware after the June 6 grain-size/LFO retune, watching `voices=...`, `Granular active=...`, `grain=...`, rhythm/load skip counters, shared-effect `dt/fb/wet/pitch`, readback spacing, and frame spikes; (2) listen-check whether the larger grain windows make the instruments richer without sounding too literal or smeary; (3) listen-check whether the restored scheduler cadence and shared-effect modulation follow particle velocity without reintroducing stalls; (4) compare default shared delay against `?audioRich=pitchDelay` and `?audioPerf=balanced` on target hardware; (5) listen-check whether dynamic rhythms feel more alive without becoming too sparse under high motion; (6) listen-check whether the attack/sustain/release and grain-LFO profiles make the six instruments easier to distinguish; (7) if stutter remains, consider a second GPU-offload pass for organism/coherence metrics so full particle readback can be reduced further; (8) validate `REGEN` behavior against `cellflow-audio-preview.html`; (9) A/B and capture short before/after clips. Defer any separate atmospheric insect-biome texture until the core six granular instruments are validated.

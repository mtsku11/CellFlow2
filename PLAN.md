...
Execute the remaining backlog in this order: (1) validate long-run continuity on phone/work-laptop hardware after the June 5 high-speed pass, watching `Granular active=...`, rhythm/load skip counters, `sync`, `drift`, and frame spikes; (2) listen-check whether dynamic rhythms feel more alive without becoming too sparse under high motion; (3) listen-check whether the new attack/sustain/release profiles make the six instruments easier to distinguish; (4) if stutter remains, consider a second GPU-offload pass for organism/coherence metrics so full particle readback can be reduced further; (5) validate `REGEN` behavior against `cellflow-audio-preview.html`; (6) A/B and capture short before/after clips. Defer any separate atmospheric insect-biome texture until the core six granular instruments are validated.

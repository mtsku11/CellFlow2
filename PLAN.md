...
Execute the remaining backlog in this order: (1) validate that stationary/near-stationary scenes no longer leave `c4` or any other color reporting a false high velocity; (2) validate long-run continuity on phone/work-laptop hardware after the June 6 grain-size/LFO retune, watching `voices=...`, `Granular active=...`, `grain=...`, rhythm/load skip counters, shared-effect `dt/fb/wet/pitch`, readback spacing, and frame spikes; (3) listen-check whether the larger grain windows make the instruments richer without sounding too literal or smeary; (4) listen-check whether the restored scheduler cadence and shared-effect modulation follow visible particle motion without reintroducing stalls; (5) compare default shared delay against `?audioRich=pitchDelay` and `?audioPerf=balanced` on target hardware; (6) listen-check whether dynamic rhythms feel more alive without becoming too sparse under high motion; (7) listen-check whether the attack/sustain/release and grain-LFO profiles make the six instruments easier to distinguish; (8) if stutter remains, consider a second GPU-offload pass for organism/coherence metrics so full particle readback can be reduced further; (9) validate `REGEN` behavior against `cellflow-audio-preview.html`; (10) A/B and capture short before/after clips. Defer any separate atmospheric insect-biome texture until the core six granular instruments are validated.

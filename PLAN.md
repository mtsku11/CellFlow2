...
- Latest maintenance pass (June 3, 2026): after trimmed assets fixed startup latency, a long-run browser check showed the remaining risk is steady-state granular node pressure under synced-organism bursts plus fast free-color clocks. The default runtime now uses a balanced audio mode with a lower active-grain cap, shorter grain runtime/release, slower summary cadence, and slower full organism readback; `?audioPerf=high` restores the previous cadence for A/B testing.

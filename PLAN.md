...
Execute the remaining backlog in this order: (1) validate long-run continuity on phone/work-laptop hardware after the low-end safe-mode pass, watching `Granular active=...`, rhythm/load skip counters, readback spacing, and frame spikes; (2) compare default safe mode against `?audioPerf=balanced` to decide how much modulation/reverb can return on target hardware; (3) listen-check whether dynamic rhythms feel more alive without becoming too sparse under high motion; (4) listen-check whether the new attack/sustain/release profiles make the six instruments easier to distinguish; (5) if stutter remains, consider a second GPU-offload pass for organism/coherence metrics so full particle readback can be reduced further; (6) validate `REGEN` behavior against `cellflow-audio-preview.html`; (7) A/B and capture short before/after clips. Defer any separate atmospheric insect-biome texture until the core six granular instruments are validated.

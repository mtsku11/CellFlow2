...
Execute the remaining backlog in this order: (1) validate long-run continuity on phone/work-laptop hardware after the June 6 safe-mode retune, watching `voices=...`, `Granular active=...`, rhythm/load skip counters, shared-effect `dt/fb/wet/pitch`, readback spacing, and frame spikes; (2) listen-check whether the restored scheduler cadence and shared-effect modulation follow particle velocity without reintroducing stalls; (3) compare default shared delay against `?audioRich=pitchDelay` and `?audioPerf=balanced` on target hardware; (4) listen-check whether dynamic rhythms feel more alive without becoming too sparse under high motion; (5) listen-check whether the new attack/sustain/release profiles make the six instruments easier to distinguish; (6) if stutter remains, consider a second GPU-offload pass for organism/coherence metrics so full particle readback can be reduced further; (7) validate `REGEN` behavior against `cellflow-audio-preview.html`; (8) A/B and capture short before/after clips. Defer any separate atmospheric insect-biome texture until the core six granular instruments are validated.

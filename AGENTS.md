...
- The default audio path now feeds the scheduler from a compact GPU summary in `audio/index.js`, while full particle readback is reserved for much slower organism refresh with in-flight guards. Color and organism speed use capped visible per-frame displacement plus a fast motion follower, so audio tempo follows screen motion rather than stale internal velocity or a duplicated control-parameter prediction. At true visible rest, color clocks park at `0` BPM rather than keeping a residual low-BPM floor alive.
...
- Granular voices now have per-color attack/sustain/release profiles and larger per-color grain windows with lightweight grain-size/start-position LFOs at distinct rates, so articulation and timbral scan movement are both part of color identity.
